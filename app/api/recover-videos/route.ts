...
export async function POST(request: NextRequest) {
  try {
    ...
    const incompleteVideos = [
      ...(pendingVideosResult?.videos || []),
      ...(processingVideosResult?.videos || [])
    ];
    ...
    console.log(`Found ${incompleteVideos.length} incomplete videos for user ${user.id} (${pendingVideosResult?.videos?.length || 0} pending, ${processingVideosResult?.videos?.length || 0} processing)`);
    ...
    for (const video of incompleteVideos) {
      try {
        console.log(`Checking video with taskId: ${video.task_id}`);
